...
if __name__ == '__main__':    
    ...
    parser.add_argument('-n',  '--number',   action='store',      dest='number',       help='path to a build folder for a default path',  type=str, default=None) 
    parser.add_argument('-d',  '--defaults', action='store_true', dest='default',      help='show default values')
    parser.add_argument('-bs', '--builds',   action='store_true', dest='show_builds',  help='show builds in the default folder')
    ...
    elif args.show_builds:
        print('Following builds are present in the default folder:')
        for build in os.listdir(Defaults.BUILDS):
            print(build)
    elif args.number:
        paths = [args.build, args.number]
        path = '/'.join(paths)
        command = f'{args.script}'
        env = set_environmental_variables(f'{path}/Bin64/nodejs', args.license, f'{path}/SourceData')
